'use strict';
angular.module('portfolio').controller('PortfolioController', ['$scope', 'Portfolio', '$http', '$rootScope', '$state', 'api','tradeSegue',
    function($scope, Portfolio, $http, $rootScope, $state, api, tradeSegue) {
        ...
        //Hijack the account dropdown to show other account type headers on a tear sheet
        //A little hacky but ¯\_(ツ)_/¯
        $scope.$watch('accountId', function(){
            if($scope.accountId === 'other'){
                $state.go('globalheader');
            }
        });
    ...
    $scope.getAccountNameById= function(accountId){
        for(var account of $scope.accounts){
